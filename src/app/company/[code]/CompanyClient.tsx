...
export default function CompanyClient({ company, initialNotes, initialMemos }: Props) {
  ...
  const generateAiSummary = async () => {
    ...
    try {
      ...
      setNotes(prev => ({
        ...
        ai_summary: data.summary ?? prev?.ai_summary ?? null,
        ai_generated_at: data.generated_at ?? prev?.ai_generated_at ?? null,
      }))
    } catch (e) {
  ...
  const saveIrUrl = async (url: string) => {
    ...
    try {
      ...
      const data = await res.json()
      setNotes(prev => ({ ...(prev ?? { code: company.code, ai_summary: null, ai_generated_at: null, my_memo: null, updated_at: null }), ...data }))
  ...
  const saveMemo = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error()
      const data = await res.json()
      setNotes(prev => ({ ...(prev ?? { code: company.code, ai_summary: null, ai_generated_at: null, ir_site_url: null, updated_at: null }), ...data }))
      setMemoSaved(true)
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 border border-line" style={{ boxShadow: '0 1px 3px rgba(27,58,91,0.06)' }}>
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs font-semibold text-sub">公式IRサイト</p>
            {!irUrlEditing && (
              <button
